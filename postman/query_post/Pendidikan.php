...
if (isset($_GET['act']) && $_GET['act'] == "excel") {
    ...
    if (isset($_FILES['filependidikan']) && $_FILES['filependidikan']['error'] == UPLOAD_ERR_OK) {
        // Validasi tipe file (hanya Excel)
        $allowedExtensions = ['xls', 'xlsx'];
        $fileExtension = pathinfo($_FILES['filependidikan']['name'], PATHINFO_EXTENSION);
        ...
        $target_dir = "../upload_excel/" . basename($_FILES['filependidikan']['name']);

        // Pindahkan file yang diunggah ke target direktori
        if (move_uploaded_file($_FILES['filependidikan']['tmp_name'], $target_dir)) {
            try {
                ...
                foreach ($Reader as $Key => $Row) {
                    ...
                    $tingkat = $konektor->real_escape_string(trim($Row[0]));
                    $indek = $konektor->real_escape_string(trim($Row[1]));
                    $gapok1 = $konektor->real_escape_string(trim($Row[2]));
                    $kenaikan = $konektor->real_escape_string(trim($Row[3]));
                    $maksimal = $konektor->real_escape_string(trim($Row[4]));

                    // Query SQL
                    $sql = "INSERT INTO pendidikan(tingkat, indek, gapok1, kenaikan, maksimal) VALUES ('$tingkat','$indek','$gapok1','$kenaikan','$maksimal')";
