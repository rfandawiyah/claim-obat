...
if (isset($_GET['act']) && $_GET['act'] == "excel") {
    ...
    if (isset($_FILES['filegudang']) && $_FILES['filegudang']['error'] == UPLOAD_ERR_OK) {
        // Validasi tipe file (hanya Excel)
        $allowedExtensions = ['xls', 'xlsx', 'csv'];
        $fileExtension = pathinfo($_FILES['filegudang']['name'], PATHINFO_EXTENSION);
        ...
        $target_dir = "../upload_excel/" . basename($_FILES['filegudang']['name']);

        // Pindahkan file yang diunggah ke target direktori
        if (move_uploaded_file($_FILES['filegudang']['tmp_name'], $target_dir)) {
            try {
                ...
                foreach ($Reader as $Key => $Row) {
                    ...
                    $kode_barang = $konektor->real_escape_string(trim($Row[0]));
                    $letak_barang = $konektor->real_escape_string(trim($Row[1]));
                    $kapasitas = $konektor->real_escape_string(trim($Row[2]));


                    // Query SQL
                   $sql = "UPDATE `databarang` SET `letak_barang`='$letak_barang',`kapasitas`='$kapasitas' where `kode_brng`='$kode_barang'";
