...
if (isset($_GET['act']) && $_GET['act'] == "excel") {
    ...
    if (isset($_FILES['filejns']) && $_FILES['filejns']['error'] == UPLOAD_ERR_OK) {
        // Validasi tipe file (hanya Excel)
        $allowedExtensions = ['xls', 'xlsx', 'csv'];
        $fileExtension = pathinfo($_FILES['filejns']['name'], PATHINFO_EXTENSION);
        ...
        $target_dir = "../upload_excel/" . basename($_FILES['filejns']['name']);

        // Pindahkan file yang diunggah ke target direktori
        if (move_uploaded_file($_FILES['filejns']['tmp_name'], $target_dir)) {
            try {
                ...
                foreach ($Reader as $Key => $Row) {
                    ...
                    $kd_jenis = $konektor->real_escape_string(trim($Row[0]));
                    $kd_kategori = $konektor->real_escape_string(trim($Row[1]));

                    // Query SQL
                    $sql = "UPDATE `jns_perawatan_inap` SET `kd_kategori`='$kd_kategori' where `kd_jenis_prw`='$kd_jenis'";
