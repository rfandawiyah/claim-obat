...
if (isset($_GET['act']) && $_GET['act'] == "excel") {
    ...
    if (isset($_FILES['filejabatan']) && $_FILES['filejabatan']['error'] == UPLOAD_ERR_OK) {
        // Validasi tipe file (hanya Excel)
        $allowedExtensions = ['xls', 'xlsx', 'csv'];
        $fileExtension = pathinfo($_FILES['filejabatan']['name'], PATHINFO_EXTENSION);
        ...
        $target_dir = "../upload_excel/" . basename($_FILES['filejabatan']['name']);

        // Pindahkan file yang diunggah ke target direktori
        if (move_uploaded_file($_FILES['filejabatan']['tmp_name'], $target_dir)) {
            try {
                ...
                foreach ($Reader as $Key => $Row) {
                    ...
                    $kd_jbtn = $konektor->real_escape_string(trim($Row[0]));
                    $nm_jbtn = $konektor->real_escape_string(trim($Row[1]));

                    // Query SQL
                    $sql = "INSERT INTO jabatan (kd_jbtn, nm_jbtn) VALUES ('$kd_jbtn','$nm_jbtn')";
