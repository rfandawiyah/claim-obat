...
if (isset($_GET['act']) && $_GET['act'] == "excel") {
    ...
    if (isset($_FILES['filekecamatan']) && $_FILES['filekecamatan']['error'] == UPLOAD_ERR_OK) {
        // Validasi tipe file (hanya Excel)
        $allowedExtensions = ['xls', 'xlsx'];
        $fileExtension = pathinfo($_FILES['filekecamatan']['name'], PATHINFO_EXTENSION);
        ...
        $target_dir = "../upload_excel/" . basename($_FILES['filekecamatan']['name']);

        // Pindahkan file yang diunggah ke target direktori
        if (move_uploaded_file($_FILES['filekecamatan']['tmp_name'], $target_dir)) {
            try {
                ...
                foreach ($Reader as $Key => $Row) {
                    ...
                    $nm_kec = $konektor->real_escape_string(trim($Row[0]));

                    // Query SQL
                    $sql = "INSERT INTO kecamatan (nm_kec) VALUES ('$nm_kec')";
