...
if (isset($_GET['act']) && $_GET['act'] == "excel") {
    ...
    if (isset($_FILES['filedepartemen']) && $_FILES['filedepartemen']['error'] == UPLOAD_ERR_OK) {
        // Validasi tipe file (hanya Excel)
        $allowedExtensions = ['xls', 'xlsx'];
        $fileExtension = pathinfo($_FILES['filedepartemen']['name'], PATHINFO_EXTENSION);
        ...
        $target_dir = "../upload_excel/" . basename($_FILES['filedepartemen']['name']);

        // Pindahkan file yang diunggah ke target direktori
        if (move_uploaded_file($_FILES['filedepartemen']['tmp_name'], $target_dir)) {
            try {
                ...
                foreach ($Reader as $Key => $Row) {
                    ...
                    $id_dep = $konektor->real_escape_string(trim($Row[0]));
                    $nama_d = $konektor->real_escape_string(trim($Row[1]));

                    // Query SQL
                    $sql = "INSERT INTO departemen (dep_id, nama) VALUES ('$id_dep','$nama_d')";
