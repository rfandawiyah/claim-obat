...
if (isset($_GET['act']) && $_GET['act'] == "excel") {
    ...
    if (isset($_FILES['filekab']) && $_FILES['filekab']['error'] == UPLOAD_ERR_OK) {
        // Validasi tipe file (hanya Excel)
        $allowedExtensions = ['xls', 'xlsx'];
        $fileExtension = pathinfo($_FILES['filekab']['name'], PATHINFO_EXTENSION);
        ...
        $target_dir = "../upload_excel/" . basename($_FILES['filekab']['name']);

        // Pindahkan file yang diunggah ke target direktori
        if (move_uploaded_file($_FILES['filekab']['tmp_name'], $target_dir)) {
            try {
                ...
                foreach ($Reader as $Key => $Row) {
                    ...
                    $nm_kab = $konektor->real_escape_string(trim($Row[0]));

                    // Query SQL
                    $sql = "INSERT INTO kabupaten (nm_kab) VALUES ('$nm_kab')";
